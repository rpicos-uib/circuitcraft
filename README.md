...
| <img src="docs/icons/xy_probe.png" width="32"> | **X-Y Oscilloscope Probe** | A second, independent probe: instead of plotting channels against time, it plots one pinned channel's voltage against another's - a real bench oscilloscope's X-Y mode, tracing Lissajous figures for phase/frequency comparisons. Right-click to pin (the older of your two pins drives X, the newer drives Y; a 3rd pin evicts the oldest), shift+right-click to unpin. Both axes share one scale, so a 90°-phase-shifted equal-amplitude pair traces an actual circle, not a stretched ellipse. Independent of the regular Probe's own pins - hold both at once to see both HUDs side by side. |
