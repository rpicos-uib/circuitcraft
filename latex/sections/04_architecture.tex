...
back faces along that axis, and its other four faces are electrically insulated. (The R2V and
V2R Converters, Section~\ref{sec:redstone-interfaces}, are the one exception: their orientation
is always fixed rather than player-chosen, since two of their faces are reserved for redstone
I/O.) Which of the figures below a given block appears in (Sections~\ref{sec:circuit-elements}
through~\ref{sec:probes}) depends only on its electrical role -- element, topology-only,
source, interface, or measurement tool -- not on any special-casing in the topology algorithm
itself, which treats all nineteen circuit-participating blocks and items uniformly.
...
into the circuit between its resolved node indices. A twentieth item, the Breadboard
introduced in Section~\ref{sec:electrician}, deliberately falls outside this set entirely: it
presents no conductive face on any side and never participates in the topology algorithm, since
its role is acquisition rather than circuit construction.
...
\subsection{Redstone-voltage interfaces}
\label{sec:redstone-interfaces}

Every source described above is gated by redstone only as a boolean on/off signal. The R2V and
V2R Converters instead exchange \emph{quantitative} information with Minecraft's own redstone
system, giving a player a way to drive the circuit's continuous voltage domain from
Minecraft's own discrete 0--15 signal-strength domain, and vice versa. Both are placed directly
on top of a Ground block -- simultaneously a placement requirement and the guarantee that their
electrical reference is always the network's true 0~V node, not an arbitrary floating one. The
R2V Converter reads two independent 0--15 redstone strengths from its north and south faces
(A and B respectively) and stamps a fixed DC voltage source of $V = A \times 16 + B$ onto its
remaining wired lead, exposing a 0--255~V range from two ordinary redstone signals -- the base
is 16, the \emph{count} of values each digit can take, not 15, the largest single value, exactly
as base-10 place value uses a multiplier of 10 rather than 9; using 15 would make the encoding
lossy; e.g.\ $A{=}0,B{=}15$ and $A{=}1,B{=}0$ would both yield $V{=}15$, an ambiguity the V2R
Converter's decoder could only ever resolve one way. The V2R Converter performs the exact inverse: it reads
whatever voltage is wired into it (as an ideal voltmeter -- it stamps nothing into the circuit
and so never loads down what it measures) and decodes it back into two 0--15 redstone outputs
on the same two faces. Chained together, the pair lets a player route a circuit's live state
back into Minecraft's own redstone contraptions -- comparators, repeaters, redstone lamps -- and
back again, without ever leaving the mod's own components.
...
\subsection{Acquisition: the Electrician villager}
\label{sec:electrician}

A learner cannot wire a circuit with components they do not yet possess, and the nineteen
blocks and items above are otherwise obtained the same way as any other vanilla item --
crafted by hand from a recipe, or found. CircuitCraft adds one further, twentieth block, the
Breadboard, whose sole purpose is to let a villager claim it as a job site and become an
\emph{Electrician}, a new villager profession that both sells a subset of the mod's own
components and buys back the raw crafting materials those components are made from, all for
emeralds, instead of any vanilla trade. Figure~\ref{fig:workshop} shows a complete Electrician's
Workshop, itself generated on demand by a bundled world-generation function
(\texttt{/function circuitcraft:electrician\_shop}) that clears its own footprint, pours its
own foundation, and places a Breadboard inside regardless of the underlying terrain, so an
instructor can seed a classroom world with a working shop in one command rather than building
one by hand.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\columnwidth]{figures/electrician_workshop.png}
\caption{A generated Electrician's Workshop, with a Breadboard placed against the back wall.}
\label{fig:workshop}
\end{figure}

Like every vanilla villager profession, the Electrician levels up (Novice through Master)
as it successfully trades, unlocking further tiers of stock rather than offering everything
from the outset. Each tier deliberately offers no more than two components to sell and no
more than two raw materials to buy -- always both, never a partial or randomized subset --
so a single villager is never a wall of trades, with one deliberate exception at the top tier
(below). The five tiers were chosen to mirror the order a learner would naturally be
introduced to the material, and each tier's buy offers are simply the crafting ingredients of
that tier's sell offers: Novice sells the resistor and capacitor (buying iron nuggets and clay
balls); Apprentice, the power supply and function generator (buying iron and copper ingots);
Journeyman, the AC Source alone (buying gold nuggets and glowstone dust); Expert, the probe
and AC probe (buying redstone and sticks); and Master offers any two of the memristor, the R2V
Converter, and the V2R Converter (buying amethyst shards and redstone, usually both) --
reserved for last as the most conceptually demanding tier. Master's sell pool of three,
randomly reduced to two per villager, is a deliberate departure from every other tier's
fully-deterministic offer: Minecraft's own trade-selection mechanism can only randomize
across one combined pool per level, so guaranteeing "any two of three" for the sell side means
the two buy items are occasionally not both guaranteed present either, a trade-off accepted
only at this one tier. The two-item cap otherwise means not every craftable item is
villager-tradeable -- the wire, ground, inductor, and the two source-adjustment modules, for
instance, remain available only through their own (simple) crafting recipes. Trades are
themselves data-driven configuration rather than code, so an instructor may re-tier or
re-price them (for instance, to gate the memristor behind a specific in-class milestone)
without modifying the mod itself.
