...
into the circuit between its resolved node indices. An eighteenth item, the Breadboard
...
A learner cannot wire a circuit with components they do not yet possess, and the seventeen
blocks and items above are otherwise obtained the same way as any other vanilla item --
crafted by hand from a recipe, or found. CircuitCraft adds one further, eighteenth block, the
...
so a single villager is never a wall of trades. The five tiers were chosen to mirror the
order a learner would naturally be introduced to the material, and each tier's buy offers are
simply the crafting ingredients of that tier's sell offers: Novice sells the resistor and
capacitor (buying iron nuggets and clay balls); Apprentice, the power supply and function
generator (buying iron and copper ingots); Journeyman, the AC Source alone (buying gold
nuggets and glowstone dust); Expert, the probe and AC probe (buying redstone and sticks); and
Master, the memristor alone (buying amethyst shards and redstone), reserved for last as the
component whose behavior depends on the integral of its own history rather than an
instantaneous relation. The two-item cap means not every craftable item is villager-tradeable
-- the wire, ground, inductor, and the two source-adjustment modules, for instance, remain
available only through their own (simple) crafting recipes. Trades are themselves
data-driven configuration rather than code, so an instructor may re-tier or re-price them
(for instance, to gate the memristor behind a specific in-class milestone) without modifying
the mod itself.
