...
to X and, if both slots were already occupied, evicting the previous X entirely. Consecutive
samples are connected with straight line segments, drawn with a small Bresenham rasterizer
since the heads-up-display drawing interface exposes no arbitrary-angle line primitive, so
the trace renders as a continuous curve rather than a scatter of isolated points.
