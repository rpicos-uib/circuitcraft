...
\textbf{A component economy gated by curricular order, not availability.} A player who can
craft every component immediately has no particular reason to encounter them in any
deliberate sequence. The Electrician villager (Section~\ref{sec:electrician}) offers an
alternative, entirely optional acquisition path whose five trade tiers are ordered to match a
typical course progression -- passive elements first, then sources, then AC analysis, then
instrumentation, with the memristor held back as the final, most conceptually demanding tier
-- so that an instructor who wants to pace a class's access to material can do so through an
in-game economy rather than a course policy enforced outside the game.
