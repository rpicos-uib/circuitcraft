...
circuit completely, verify it, \emph{then} switch the bench supply on) rather than a
schematic tool's power sources, which are implicitly ``live'' the instant they are drawn.
It also gives a player room to make, and recover from, the same wiring mistake a real bench
technician must learn to avoid: an ideal voltage source whose two leads both land on the
same node is shorted across itself regardless of whether it happens inside a game or a real
circuit, and the solver's defensive handling of that case
(Section~\ref{sec:circuit-solver}) means the mistake surfaces as an in-game warning to fix,
not a crash.
