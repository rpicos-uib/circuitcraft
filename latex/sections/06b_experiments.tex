...
the shift-right-click value editor. A power supply, function generator, or AC Source
additionally remains disconnected from whatever it would otherwise drive until it receives a
redstone signal: an open circuit rather than a 0~V source for the first two in the transient
solver, or a silenced 0~V excitation for the AC Source during a sweep
(Sections~\ref{sec:circuit-solver} and~\ref{sec:ac-analysis}). Every bench in this set
therefore includes a lever placed directly on top of its source block, left off by default --
marking a clean $t=0$ for whichever transient a time-domain experiment observes, or simply
gating whether an AC Source's sweep shows anything at all -- and flipping it is accordingly
the first step of every procedure below.
...
(a hint confirms it is pinned), then right-click the capacitor to run the sweep and display
the Bode plot.
...
\textbf{Procedure.} Flip the lever. Right-click the AC probe against the AC Source, then
right-click the capacitor to run the sweep.
...
\textbf{Procedure.} Flip the lever. Right-click the AC probe against the AC Source, then
right-click the op-amp block itself (probing a three-terminal component directly reads its
output node's voltage, exactly as the ordinary time-domain probe already does for it) to run
the sweep.
